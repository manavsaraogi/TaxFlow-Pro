        ...
        PartA_GEN2: {
          ...
          ...(Array.isArray(gen.members) && gen.members.length > 0 ? {
            PartnerOrMemberInfo: gen.members.map((m: any) => ({
              ...
              AddressDetailWithZipCode: {
                AddrDetail:            ([m.flatNo, m.buildingName, m.streetName, m.localityOrArea].filter(Boolean).join(', ') || m.address || client.address || '-').toUpperCase(),
                CityOrTownOrDistrict:  (m.cityOrTownOrDistrict || m.city || client.city || 'Delhi').toUpperCase(),
                StateCode:             m.stateCode ?? client.state ?? '09',
                CountryCode:           m.countryCode ?? '91',
                PinCode:               Number(m.pinCode) || Number(client.pinCode) || 110001,
              },
