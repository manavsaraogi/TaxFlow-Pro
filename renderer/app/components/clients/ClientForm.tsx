    ...
    async function load() {
      ...
      try {
        ...
        if (isMock) {
          ...
          data = { ...EMPTY_FORM, pan: 'ABCPK1234E', fullName: 'Priya Kapoor', assesseeType: 'INDIVIDUAL' };
        } else {
          const res = await (window as any).taxflow.clients.get(clientId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isMock) {
        ...
        result = { success: true, data: { id: clientId ?? 99 } };
      } else if (isEdit) {
        result = await (window as any).taxflow.clients.update(clientId, {
          ...form,
          pan: form.pan.toUpperCase(),
          pinCode: form.pinCode ? parseInt(form.pinCode) : undefined,
          dateOfBirth: form.dateOfBirth || undefined,
        });
        result = result.success ? { ...result, data: { id: clientId } } : result;
      } else {
        // Get firmId from auth store / firm info
        const firmRes = await (window as any).taxflow.auth.getFirmInfo();
        const firmId = firmRes?.data?.id ?? 1;

        result = await (window as any).taxflow.clients.create({
          firmId,
          pan: form.pan.toUpperCase(),
