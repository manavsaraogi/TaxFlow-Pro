...
export function ClientDetail({ clientId, onNavigate }: ClientDetailProps) {
  ...
  async function loadClient() {
    ...
    try {
      ...
      const result = await window.taxflow.clients.get(Number(clientId));
...
function NewReturnModal({ client, onClose, onCreated }: {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const result = await window.taxflow.returns.create({
        clientId: client.id,
        ayLabel,
        formType,
        regime: regime as any,
        filingType: 'ORIGINAL' as any,
      });
      if (!result.success) throw new Error((result as any).error ?? 'Failed');
...
function AddBankModal({ clientId, onClose, onAdded }: { clientId: number; onClose: () => void; onAdded: () => void }) {
  ...
  async function handleAdd() {
    ...
    setLoading(true);
    try {
      if (typeof window.taxflow === 'undefined') { onAdded(); return; }
      const result = await window.taxflow.clients.addBankAccount(clientId, form);
      if (!result.success) throw new Error((result as any).error ?? 'Failed');
      onAdded();
    } catch (e: any) {
      setError(e.message);
