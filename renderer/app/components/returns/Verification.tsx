...
export default function Verification({
    ...
    async function load() {
      ...
      try {
        if (typeof window.taxflow === 'undefined') {
          ...
          setData(MOCK_VERIFICATION);
          setShowTRP(!!MOCK_VERIFICATION.TRPName);
        } else {
          const res = await window.taxflow.returns.get(returnId);
          if (res.success && res.data?.verification) {
            const v = res.data.verification as VerificationType;
            setData(v);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      if (typeof window.taxflow !== 'undefined') {
        const res = await window.taxflow.returns.upsertVerification?.(returnId, data);
  ...
  const handleFile = useCallback(async () => {
    ...
    try {
      if (typeof window.taxflow !== 'undefined') {
        ...
        await window.taxflow.returns.upsertVerification?.(returnId, data);
        // Mark as filed
        const res = await window.taxflow.returns.updateStatus(returnId, 'FILED');
        if (!res.success) throw new Error(res.error ?? 'Filing failed');
      }
