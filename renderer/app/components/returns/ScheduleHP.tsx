...
const isMock =
  ...
  typeof (window as any).taxflow === 'undefined';
...
export default function ScheduleHPComponent({ returnId, returnData, onSaved, setDirty }: Props) {
  ...
  const save = useCallback(async (props: PropertyEntry[]) => {
    ...
    try {
      ...
      if (!isMock) {
        // window.taxflow.returns.upsertHP would be the canonical channel;
        // fall back to console until IPC is wired
        await (window as any).taxflow.returns.upsertHP?.(returnId, payload);
      } else {
        await new Promise((r) => setTimeout(r, 400));
        console.log('[mock] upsertHP', returnId, payload);
      }
