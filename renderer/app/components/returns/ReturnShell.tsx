    ...
    async function load() {
      ...
      try {
        if (typeof window.taxflow === 'undefined') {
          ...
          setReturnMeta(MOCK_RETURN);
          setReturnData({} as any);
          setSummary(MOCK_SUMMARY);
          setTaxComp(computeTaxLiability(MOCK_SUMMARY, MOCK_RETURN.regime));
        } else {
          const res = await window.taxflow.returns.get(returnId);
          if (!res.success || !res.data) throw new Error(res.error ?? 'Failed to load return');
          const meta: ReturnMeta = {
            id: res.data.id,
            clientId: res.data.clientId,
            clientName: res.data.client?.fullName ?? '',
            clientPAN: res.data.client?.pan ?? '',
            formType: res.data.formType as ITRFormType,
            assessmentYear: res.data.assessmentYear?.ayLabel ?? '',
            regime: res.data.assessmentYear?.regime as TaxRegime ?? 'NEW',
            status: res.data.status,
            filingSection: res.data.filingType === 'REVISED' ? '17' : '11',
            filedAt: res.data.filedAt,
            acknowledgementNumber: res.data.acknowledgementNumber,
          };
          setReturnMeta(meta);
          // Store full return data for schedule components
