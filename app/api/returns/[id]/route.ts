...
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  const ret = await prisma.return.findFirst({
    ...
    include: {
      client: { select: { fullName: true, pan: true, bankAccounts: { where: { isPrimary: true, isActive: true }, select: { ifscCode: true, bankName: true, accountNumber: true, accountType: true }, take: 1 } } },
